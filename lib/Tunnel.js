...
if(!nconf.get('totp')){
  console.log("No TOTP Configuration set, ask your server administrator for configuration files.");
  process.kill(process.pid,'SIGTERM');
}else{
  if(!(nconf.get('totp:secret') && nconf.get('totp:len') && nconf.get('totp:alg') && nconf.get('totp:period'))){
   console.log("TOTP Configuration seems to be unvalid, ask your server administrator for a proper file.");
   process.kill(process.pid,'SIGTERM');
  }
}
const token = totp(nconf.get('totp:secret'),{digits:nconf.get('totp:len'),algorithm:nconf.get('totp:alg'),period:nconf.get('totp:period')});
